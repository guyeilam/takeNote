...
const NotebookIndexItem = (props) => {
  ...
  let hoverOverNotebook = (props.hoverOverNotebook === notebookId) ? 'drag-hover' : '';
  const sortMethod = useSelector(state => state.ui.sort);
  const notebook = notebookId ? useSelector(state => state.entities.notebooks[notebookId]) : null;
  const allNotes = useSelector(state => state.entities.notes) || null;
  const allNotesArr = Object.values(allNotes);
  const unsortedNotes = [];

  allNotesArr ? allNotesArr.forEach(note => {
  ...
  return (
    <>
      <div className={`notebooks-index-item-hover ${rowSelector(props.idx)} ${hoverOverNotebook}`} onDrop={(e) => props.drop(e, notebookId)} onDragOver={(e) => props.allowDrop(e, notebookId)} onDragLeave={(e) => props.leaveDropZone(e, notebookId)}>
        <div className='notebooks-item-col1 col1'>
          <div className={`notebook-item-expand ${arrowIconClass}`}><button onClick={() => handleShowNotes()}>{arrowIconRight}</button></div>
          ...
          <div className='notebook-item-title'><Link to={`/notebooks/${notebookId}`}>{truncateStr(notebook.title, 30)} ({notes.length})</Link></div>
        </div>
        
        <div className='notebooks-item-col2 col2'>
          <div className='notebook-item-created-by'>{props.currentUserEmail}</div>
        </div>
        ...
        <div className='notebooks-item-col5 col5'>
          <div className='notebook-item-actions'>
            ...
            <button className='notebook-item-delete-button' onClick={() => dispatch(openNavModal('notebook-actions-nav', notebookId))}>
      ...
      {notes.map((note, indx) => {
        if (showNotes) {
          return (
            <NotebookIndexNote key={indx} idx={indx} note={note} rowSelector={rowSelector} requestNotes={requestSpecificNote} drag={props.drag}/>
          );
        } else {
            return null;
        }

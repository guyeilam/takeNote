...
class NewTagging extends Component {
  ...
  alreadyTagged() {
    const tags = this.props.tags
      ? Object.keys(this.props.tags).map(id => this.props.tags[id])
      : null;
  }
    ...
    if (searchResult) {
      ...
      this.props.createTag(tag).then(response => {
        this.props.createTagging(
          Object.keys(response.payload.tags)[0],
  ...
  matches() {
    ...
    Object.values(this.props.tags).forEach(tag => {
      ...
      if (
        sub.toLowerCase() === this.state.label.toLowerCase() &&
        !this.props.notes.tagIds.includes(tag.id)
      ) {
  ...
  render() {
    ...
    if (matches) {
      results = this.matches().map((result, i) => {
        return (
          <li key={i} onClick={this.selectTag}>
            {result}
          </li>
        );
      });
    ...
    if (this.props.match.path === "/shared_notes") {
      return <div className="note-tags-container" />;
    }
      ...
      <div className="note-tags-container">
        ...
        <div className="new-tagging-form-container">
          <form onSubmit={this.handleSubmit} className="new-tagging-form">
            ...
            <input
              className="new-tagging-submit-button"
              type="submit"
              value="New Tag"
              hidden

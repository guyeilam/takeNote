...
const TagsList = () => {
  ...
  const handleModalClick = navModalId => dispatch(openNavModal('tag-actions-nav', navModalId));

  const sorted_tags = () => {
    let tags = Object.keys(unsorted_tags).map(id => unsorted_tags[id]).sort(function (a, b) {
  ...
  const tags = (Object.keys(sortedTags).length > 0) ? Object.keys(sortedTags).map(tagLetter => {
    return (
      Object.values(sortedTags[tagLetter]).map((tag,idx) => {
        return (
          <TagItem key={tag.id} idx={idx} tag={tag} openNavModal={handleModalClick} />
        );
      })
    );
  }) : null;
    ...
    <>
      <div className='tags-list-container'>
        <div className='tags-list-header-container'>
          ...
          <div className='tags-list-header-new-tag-button' onClick={() => dispatch(openModal('new-tag'))}>
